...
// Brand frame (docs/brand/BRAND_IDENTITY.md §7.4): 480px white card on Surface Muted, hairline border, ink ramp.
const WRAP_OPEN = `<div style="background:#f9f9fb;padding:24px 12px"><div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;background:#ffffff;border:1px solid #e5e5e5;border-radius:12px;padding:24px;color:#121212">`;
...
const REPLY_HTML = `<p style="color:#737373;font-size:12px">Something not working, or an idea for Perk? Reply to this email and it reaches a person.</p>`;
...
<p style="color:#525252">Tap the button to sign in on ${escapeHtml(p.host)}. The link works once and expires in 24 hours.</p>
${button(p.url, "Sign in", "#121212")}
<p style="color:#525252;font-size:14px">If you didn't request this you can ignore it.</p>
<p style="color:#737373;font-size:12px">${escapeHtml(spamText)}</p>
...
<p style="color:#525252">Open it anytime from this link. Bookmark it or add it to your home screen.</p>
${button(url, "Open my card", shop.brandColor)}
<p style="color:#525252;font-size:14px">Collect ${shop.stampsRequired} stamps to earn: ${escapeHtml(shop.rewardText)}.</p>
<p style="color:#737373;font-size:12px">Sent by Perk on behalf of ${escapeHtml(shop.name)}.</p>
${REPLY_HTML}${WRAP_CLOSE}`;
  return { subject, text, html };
}
