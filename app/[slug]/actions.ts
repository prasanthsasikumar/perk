...
export async function getOrCreateCard(slug: string): Promise<{ cardId: string; shopId: string }> {
  ...
  if (existing && (await getCard(db, shop.id, existing))) {
    ...
    return { cardId: existing, shopId: shop.id };
  }
  ...
  return { cardId: card.id, shopId: shop.id };
}
...
export async function issueApple(slug: string, then?: string, token?: string): Promise<void> {
  const { cardId } = await getOrCreateCard(slug);
